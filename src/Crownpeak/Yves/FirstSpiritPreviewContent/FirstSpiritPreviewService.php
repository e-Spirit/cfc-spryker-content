...
class FirstSpiritPreviewService
{
  ...
  public function isPreviewAuthenticationRequested(Request $request): void
  {
    ...
    if (!$this->isPreview()) {
      ...
      $this->getLogger()->debug('[FirstSpiritPreviewService] Checking for preview mode... Given: ' . $receivedToken . ', Configured: ' . $configuredToken);

      if ($receivedToken === $configuredToken) {
        $this->getLogger()->debug('[FirstSpiritPreviewService] Preview mode enabled based on token');
    ...
    $this->getLogger()->debug('[FirstSpiritPreviewService] Referers - Request: ' . $refererFromRequest . ', Session: ' . $refererFromSession);
    ...
    if (empty($refererFromRequest)) {
      ...
      // $this->getLogger()->debug('[FirstSpiritPreviewService] Initial request detected, clearing stored referer');
      ...
      $this->getLogger()->debug('[FirstSpiritPreviewService] Using referer from request: ' . $refererFromRequest);
      ...
      $this->getLogger()->debug('[FirstSpiritPreviewService] Using referer from session: ' . $refererFromSession);
